...
function aplicarSort(data) {
  ...
  return [...data].sort((a, b) => {
    ...
    if (va == null) va = '';
    if (vb == null) vb = '';
    if (field === 'creado_en') return (new Date(va) - new Date(vb)) * mul;
...
function renderClientes() {
  ...
  tbody.innerHTML = data.map(c => `
    <tr>
      ...
      <td style="font-size:12px;">${new Date(c.creado_en).toLocaleDateString('es-AR')}</td>
  ...
  try {
    ...
    document.getElementById('detalleContenido').innerHTML = `
      ...
      <div style="margin-top:16px;">
        ...
        <div class="detail-row"><span class="detail-label">Creado:</span> ${new Date(c.creado_en).toLocaleDateString('es-AR')}</div>
      </div>
